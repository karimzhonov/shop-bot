...
from .exceptions import NotInitedException, WrongCallbackException
from .utils import *
...
class InlineTimepicker:
    ...
    def get_keyboard(self, chat_id: Optional[int] = None) -> types.InlineKeyboardMarkup:
        ...
        if curr_date_time + an_hour_date_time <= max_date_time:
            rows[0].append(create_inline_callback_button('↑', InlineTimepicker.CALLBACK_HOUR_INCREASE))
        else:
            rows[0].append(create_inline_callback_button(' ', InlineTimepicker.CALLBACK_WRONG_CHOICE))

        if curr_date_time + minute_step_date_time <= max_date_time:
            rows[0].append(create_inline_callback_button('↑', InlineTimepicker.CALLBACK_MINUTE_INCREASE))
        else:
            rows[0].append(create_inline_callback_button(' ', InlineTimepicker.CALLBACK_WRONG_CHOICE))

        if curr_date_time - an_hour_date_time >= min_date_time:
            rows[2].append(create_inline_callback_button('↓', InlineTimepicker.CALLBACK_HOUR_DECREASE))
        else:
            rows[2].append(create_inline_callback_button(' ', InlineTimepicker.CALLBACK_WRONG_CHOICE))

        if curr_date_time - minute_step_date_time >= min_date_time:
            rows[2].append(create_inline_callback_button('↓', InlineTimepicker.CALLBACK_MINUTE_DECREASE))
        else:
            rows[2].append(create_inline_callback_button(' ', InlineTimepicker.CALLBACK_WRONG_CHOICE))

        rows[1].extend([
            create_inline_callback_button(user_info.current_time.hour, InlineTimepicker.CALLBACK_WRONG_CHOICE),
            create_inline_callback_button(user_info.current_time.minute, InlineTimepicker.CALLBACK_WRONG_CHOICE)
        ])
        rows[-1].append(create_inline_callback_button('OK', InlineTimepicker.CALLBACK_SUCCESS))
